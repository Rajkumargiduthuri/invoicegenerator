...
if (isset($_POST["submit"])) {
    ...
    if ($conn->query($sql)) {
        if (isset($_POST["Sname"]) && is_array($_POST["Sname"])) {
            ...
            if (!empty($updateRows)) {
                $updateSql = "INSERT INTO quservice (Id, Sid, Sname, Description, Qty, Price, Totalprice, Discount, Finaltotal) VALUES ";
            ...
            // Insert new rows
            if (!empty($insertRows)) {
                $insertSql = "INSERT INTO quservice (Sid, Sname, Description, Qty, Price, Totalprice, Discount, Finaltotal) VALUES ";
                $insertSql .= implode(",", $insertRows);
                $conn->query($insertSql);
            }

            echo "<SCRIPT>
            window.alert('quotation Updated successfully;')
